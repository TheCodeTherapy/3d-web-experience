...
const REACT_MML_DOCUMENTS_DIR = path.join(dirname, "../react-mml-documents");
const reactMmlDocumentServer = new ReactMMLDocumentServer(REACT_MML_DOCUMENTS_DIR);
...
app.ws("/mml-document", (webSocket: ws.WebSocket) => {
  // Default to 'index' document for backward compatibility
  reactMmlDocumentServer.handle(webSocket, "index");
});

app.ws("/mml-document/:documentName", (webSocket: ws.WebSocket, req: express.Request) => {
  const documentName = req.params.documentName;
  reactMmlDocumentServer.handle(webSocket, documentName);
});

// --- React MML Document API Endpoints ----------
app.get("/api/react-mml-documents", (req, res) => {
  const availableDocuments = reactMmlDocumentServer.getAvailableDocuments();
  res.json({ documents: availableDocuments });
});
